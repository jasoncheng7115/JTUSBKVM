...
3. 請以 **系統管理員身份** 執行 start_kvm.bat，過程中會引導需要安裝 Python 與 OpenSSL
...
在採用本機執行時，每次執行 start_kvm 會自動先連接本 GitHub 程式庫，如果有新版即會自動下載更新；若沒有網路可用則會採用先前已經下載過的本機版本運作。
...
使用 RJ45 Console 線模式時，請先將本裝置接上控制端電腦 (來源端)，通電啟動 Arduino 後，再將 RJ45 Console 線接照目標裝置。
